...
- 11444 피보나치수6                : 행렬 곱셈 + 분할정복
- 2206 벽부수고이동하기             : 상태에 따른 BFS
- 1707 이분 그래프                 : 문제에서 주어진 이분 그래프의 특성을 활용하여 BFS 사용하기
- 11657 타임머신                   : 벨만-포드 알고리즘 활용 N-1번 모든 간선을 체크하고 N번째 무한사이클이 있는지 체크
---
...
7. DFS (깊이 우선 탐색)
   - 깊이 우선 탐색 : 주로 재귀함수나 stack을 활용하여 구현
   - 모든 노드를 방문하고자 할 때 주로 사용
   - BFS보다 구현이 비교적 간결하지만, 최단 경로를 보장하지는 않는다.
   - 경로의 특징을 저장해야 하는 문제(예: 경로에 특정 숫자가 포함되면 안 됨)

8. BFS (너비 우선 탐색)
   - 주로 queue를 사용해서 구현
   - 최단경로를 찾는 문제에서 사용됨.(가중치 x)

DFS, BFS 시간복잡도 :
인접 리스트(vector<int> graph[MAX]) : O(V + E)
행렬 (int graph[MAX][MAX]) : O(V^2)

9. 위상정렬
   - 순서가 정해져 있는 작업을 순서대로 나열하는 알고리즘
   - ex) 선수과목(수학1 -> 수학2), 옷입기(양말 -> 신발), 조립 순서 등
   - 방향 그래프여야만 하며, 사이클이 없어야 한다. (DAG)
   - 핵심 키워드 : 진입 차수(Indegree)
      - 나에게 들어오는 화살표의 개수 (= 나보다 먼저 해야 할 작업의 수)
      - Indegree == 0 : 선행 조건이 없는 상태. 즉, 지금 바로 수행해도 되는 작업
   - 알고리즘 흐름 : 
   준비 : 모든 노드의 진입 차수 계산 -> 
   시작 : 진입 차수가 0인 노드 큐에 넣기 -> 
   반복 : 큐가 빌 때까지 아래 항목 반복
      1. 노드(cur)꺼내서 결과에 추가
      2. cur와 연결된 간선 모두 제거(next의 indegree 감소) 
      3. next의 indegree == 0 이면 큐에 추가

10. 다익스트라 알고리즘
   - 현재 상황에서 가장 이득이 되는 선택만 쫓는 그리디 알고리즘
   - 조건 : 반드시 모든 간선의 가중치가 양수이어야 한다.
   - 동작원리 : 
      1. 출발점의 비용을 0으로 두고, 우선순위 큐에 넣는다.
      2. 큐에서 비용이 가장 작은 노드를 꺼낸다. (우선순위 큐를 오름차순으로 정렬)
      3. 해당 노드와 연결된 노드를 확인하며 기존에 알던 경로보다 적은 비용이 있으면 갱신하고 큐에 넣는다.
      4. 큐가 빌 때까지 반복

11. 벨만-포드 알고리즘
   - 혹시 모를 변수(음수 간선)까지 모두 고려하기 위해 전체를 계속해서 뒤지는 턴(Turn) 기반 알고리즘
   - 조건 : 간선의 가중치가 음수여도 정상 동작
   - 동작원리 :
      1. 출발점의 비용을 0으로 둔다.
      2. 그래프에 존재하는 "모든 간선"을 훑으며 비용을 갱신하는 과정을 N-1번 반복한다. (정상적인 그래프라면 여기서 최단거리 확정)
      3. 마지막으로 N번째 훑기를 진행하고 비용이 갱신되면 무한 사이클이 존재한다고 확정

12. 플로이드-워셜 알고리즘
   - 모든 정점에서 모든 정점까지의 최단 거리를 구하는 알고리즘
   - 핵심 : 경유지(k)를 거쳐 갈 것인가? i -> j 와 i -> k -> j 중 최단 거리를 선택
      갱신 점화식 : dist[i][j] = min(dist[i][j], dist[i][k] + dist[k][j]);
   - 동작원리 :
      1. 자기 자신으로 가는 칸은 0, 나머지 노드로 가는 칸은 INF(무한대)로 초기화
      2. 주어진 간선 비용을 배열에 넣는다. dist[u][v] = min(dist[u][v], cost);
      3. 3중 루프 실행, 가장 바깥쪽 루프가 경유지(k)여야 함.
